...
if __name__ == "__main__":
    ...
    (cnt, hierarchy) = cv2.findContours(img.astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    ...
    for idx in range(len(cnt)):
        if cv2.contourArea(cnt[idx]) > 0:
            ...
            if 0.1 < circularity < 0.9:
                ...
                count += 1
                print(cv2.boundingRect(cnt[idx])[2])
                cv2.putText(f_img, f"{count}", [int(M['m10']/M['m00']) + cv2.boundingRect(cnt[idx])[2], int(M['m01']/M['m00'])],
